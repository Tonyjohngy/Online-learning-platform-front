      ...
      <el-form :model="video" label-width="120px">
        ...
        <el-form-item label="上传视频">
          <el-upload
            ...
            :file-list="fileList"
            :action="BASE_API+'/eduvod/video/updateVideo'"
            :limit="1"
            class="upload-demo">
...
export default {
  data() {
    return {
      ...
      dialogChapterFormVisible: false, // 章节添加弹窗
      dialogVideoFormVisible: false, // 小节添加弹窗
  ...
  methods: {
    ...
    // 删除提交视频小节
    handleVodRemove() {
      videoApi.removeVideoById(this.video.videoSourceId)
        .then(response => {
          // 提示信息
          this.$message({
            type: 'success',
    ...
    saveChapter() {
      ...
      chapterApi.addVideoChapter(this.chapter).then((response) => {
    ...
    updateChapter() {
      chapterApi.updateChapterVideo(this.chapter).then((response) => {
        this.dialogChapterFormVisible = false
        this.$message({
          type: 'success',
          message: '修改章节成功'
    ...
    removeChapter(chapterId) {
      this.$confirm('此操作将删除章节，是否继续？', '提示', {
        ...
        chapterApi.removeChapter(chapterId).then((response) => {
          this.getChapterVideo()
        })
      )
    }
