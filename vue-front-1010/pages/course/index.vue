            ...
            <dd class="c-s-dl-li">
              <ul class="clearfix">
                <li v-for="(item,index) in subSubjectList" :key="index" :class="{active:twoIndex==index}">
                  <a title="" href="#" @click="searchTwo(item.id,index)">{{ item.title }}</a>
                </li>
              </ul>
            </dd>
          </dl>
...
export default {
  ...
  methods: {
    // 初始化课程信息
    initCourse() {
      courseApi.getCourse(1, 8, this.searchObj).then((response) => {
    ...
    gotoPage(current) {
      ...
      courseApi.getCourse(current, 8,this.searchObj).then((response) => {
        this.data = response.data.data
      })
    },
    // 获取课程分类
